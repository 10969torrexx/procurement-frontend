        ...
        <div class="table-responsive">
            <table class="text-center ppmp" style=" line-height:22px;font-size:15px">
                <thead>
                    ...
                    <tr >
                        <td style="font-size: 15px;" class="text-dark text-bold-600" >{{ $data[0]->code }}</td>
                ...
                {{-- @foreach($data as $data) --}}
                   @php $e = 0 @endphp
                    @for ($i = 0; $i < count($data); $i++)
                        @php $e += $data[$i]->estimated_price @endphp
                        <tbody class="tbody">
                            <tr>
                                ...
                                {{-- <td style="text-align: left; color:blue;" id="status{{ $i }}"> --}}
                                     <?php
                                        if($data[$i]->status == 1){
                                    ?>
                                <td style="text-align: left; color:blue;" id="status{{ $i }}">Pending</td>
                                <?php
                                        }else if($data[$i]->status == 2){
                                ?>
                                <td style="text-align: left; color:green;" id="status{{ $i }}">Approved</td>
                                <?php
                                        }if($data[$i]->status == 3){
                                ?>
                                <td style="text-align: left; color:red;" id="status{{ $i }}">Disapproved</td>
                                       <?php }?>
                                {{-- </td> --}}
                                <td colspan="16"> </td>
                            </tr>
                            <tr >
                                ...
                                <td>
                                    ...
                                    <i class="fa-solid fa-pen-to-square edit" data-index="{{ $i }}" title="Edit"></i></div><br>
                                    ...
                                    <div class="mb-1" id="action{{ $i }}">
                                        <button class="btn btn-outline-success approve button" data-toggle ="{{ $data[$i]->id }}" data-id="{{ $data[$i]->pt_id }}" value="{{ $i }}" data-index="{{ $i }}" > &nbsp; &nbsp;Approve &nbsp;</button>
                                        <button class="btn btn-outline-danger disapprove button" data-toggle ="{{ $data[$i]->id }}" data-id="{{ $data[$i]->pt_id }}" value="{{ $i }}" data-index="{{ $i }}">Disapprove</button> 
                                    </div>
                                </td>
