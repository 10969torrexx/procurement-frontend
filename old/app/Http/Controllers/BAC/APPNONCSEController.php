...
class APPNONCSEController extends Controller
{
      public function index(){
        ...
        $Categories = DB::table("ppmps as p")
            ...
            ->where("pt.project_year","=",date("Y") + 1)
            ->where("p.app_type", 'Non-CSE')
            ->where("p.is_supplemental", 0)
            ->where("p.status", "=", 4)
            ->where("pt.status", "=", 4)
        ...
        $Project_title = DB::table("project_titles as pt")
              ...
              ->where("p.is_supplemental", 0)
              ->where("p.status", "=", 4)
              ->where("pt.project_year",$Categories[0]->project_year)
              ->groupBy("pt.project_year")
              ->get("pt.project_year");

        $signatures = DB::table("signatories_app_non_cse")
              ->where("campus",session('campus'))
              ->where("Year",$Categories[0]->project_year)
              ->get();

        $prepared_by = DB::table("signatories_app_non_cse")
              ->where("campus",session('campus'))
              ->where("Year",/* date("Y") */ $Categories[0]->project_year)
              ->where("Role","=",1)
              ->get();
      ...
      public function generatepdf(Request $request){
        ...
        try{
          ...
          // dd( $prepared_by);
          // if(empty($signatures) || empty($prepared_by) || empty($recommending_approval) || empty($approved_by)){
      ...
      //       ->orderBy("p.project_code", "ASC")
      //       ->get();
      //   return Excel::download(new APP_Non_CSEExport($Categories, $ppmps), 'APP_NON-CSE.xlsx');
      // }

      // public function export(){

      //   return Excel::download(new APP_Non_CSEExport, 'APP-NON-CSE.xlsx');
      // }
      public function try(){
          $Categories = DB::table("ppmps")
          ->whereNull("deleted_at")
